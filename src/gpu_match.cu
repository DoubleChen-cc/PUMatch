      ...
      for (int i = start_ptr; i < pat->rowptr[level + 1]; i++) {
        ...
        if (pat->set_ops[i] & 0x20) {
          for (graph_node_t k = 0; k < arg[wid].num_sets; k++) {
            ...
            if(t>idx){
              if(n_size<=DEGREE_THRESHOLD){
                arg[wid].set1[k] = &g_managed->colidx[g_managed->rowptr[t]];
                arg[wid].set1_size[k] = n_size;
              }else{
              ne = get_neighbors_from_buffer(t,*d_buffer);
              if(ne.neighbor_num == 0){
                switched = add_vertex_block(t, block_queue, cpu_visible_queue, write_block_flags, flags);
                
                insertBranch(tree, stk, pat, level-1, k, t);
                if(switched){
                  Buffer g_buffer = *d_buffer;
                  bool updated = false;
                  if(tid == 0){
                    
                    if (atomicCAS(&flags->block_update_token[blockIdx.x], 0, 1) == 0) {
                      
                      update_buffer(g_buffer, flags);
                      atomicExch(&flags->block_update_token[blockIdx.x], 0);
                      updated = true;
                    }
                  }
                    
                  updated = __shfl_sync(0xFFFFFFFF, updated, 0);
                  if(!updated){
                    return false;
                  }
                  ...
                  ne = get_neighbors_from_buffer(t,*d_buffer);
                  if (ne.p_index >= 0) {
                    lruAccess_bucket(d_buffer, ne.p_index);
                  }
                  arg[wid].set1[k] = ne.neighbor_ptr;
                  arg[wid].set1_size[k] = ne.neighbor_num;
                }else{
                  return false;
                }                
              }else{
                ...
                if(t>idx){
                  ...
                  if(ne.neighbor_num == 0){
                    ...
                    if(switched){
                      ...
                      arg[wid].set2[k] = ne.neighbor_ptr;
                      arg[wid].set2_size[k] = ne.neighbor_num;
                    }else{
                      return false;
                    }                
                  }else{
