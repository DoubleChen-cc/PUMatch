...
namespace STMatch {
  ...
  __device__ void compute_set(Arg_t* arg) {
    ...
    for (int idx = tid; (idx < ((size_psum[wid][WARP_SIZE] > 0) ? (((size_psum[wid][WARP_SIZE] - 1) / WARP_SIZE + 1) * WARP_SIZE) : 0) && still_loop); idx += WARP_SIZE) {
      ...
      if (idx < size_psum[wid][WARP_SIZE]) {
        ...
        if(LABELED){
        bitarray32 lb = arg->pg->vertex_label[arg->set1[slot_idx][offset]];
      ...
      if (slot_idx < __shfl_down_sync(0xFFFFFFFF, slot_idx, 1)) {
        int increment = __popc(predicate & ((1 << (tid + 1)) - (1 << prev_idx)));
        // 边界检查：确保 end_pos 不会超出 GRAPH_DEGREE
        if (end_pos[wid][slot_idx] + increment < GRAPH_DEGREE) {
          end_pos[wid][slot_idx] += increment;
        }else{
          end_pos[wid][slot_idx] = GRAPH_DEGREE;
        }
      }
      else if (tid == WARP_SIZE - 1 && slot_idx < arg->num_sets) {
        int increment = __popc(predicate & (0xFFFFFFFF - (1 << prev_idx) + 1));
        // 边界检查：确保 end_pos 不会超出 GRAPH_DEGREE
        if (end_pos[wid][slot_idx] + increment < GRAPH_DEGREE) {
          end_pos[wid][slot_idx] += increment;
        }else{
  ...
  __device__ void extend(Graph* g, PartialGraph* pg, Pattern* pat, CallStack* stk, JobQueue* q, pattern_node_t level) {
        ...
        if (pat->set_ops[i] & 0x20) {
          for (graph_node_t k = 0; k < arg[wid].num_sets; k++) {
            ...
            if (!EDGE_INDUCED) {
              ...
              if(pg->rowptr[t+1] - pg->rowptr[t]==0){
                arg[wid].set2[k] = &g->colidx[g->rowptr[t]];
                arg[wid].set2_size[k] = (graph_node_t)(g->rowptr[t + 1] - g->rowptr[t]);
              }else{
                arg[wid].set2[k] = &pg->colidx[pg->rowptr[t]];
                arg[wid].set2_size[k] = (graph_node_t)(pg->rowptr[t + 1] - pg->rowptr[t]);
              }
            }
            ...
            if(pg->rowptr[t+1] - pg->rowptr[t]==0){
              ...
              arg[wid].set1[k] = &pg->colidx[pg->rowptr[t]];
              arg[wid].set1_size[k] = (graph_node_t)(pg->rowptr[t + 1] - pg->rowptr[t]);
            }
          ...
          if (pat->set_ops[i] & 0x40) { //INTE
            ...
            for (graph_node_t k = 0; k < arg[wid].num_sets; k++) {
              graph_node_t t = path(stk, pat, level - 1, k);
              graph_node_t* neighbor = &pg->colidx[pg->rowptr[t]];
              graph_node_t neighbor_size = (graph_node_t)(pg->rowptr[t + 1] - pg->rowptr[t]);
              if(pg->rowptr[t+1] - pg->rowptr[t]==0){
            ...
            for (graph_node_t k = 0; k < arg[wid].num_sets; k++) {
              ...
              if (!EDGE_INDUCED) {
                ...
                if(pg->rowptr[t+1] - pg->rowptr[t]==0){
                  ...
                  neighbor = &pg->colidx[pg->rowptr[t]];
                  neighbor_size = (graph_node_t)(pg->rowptr[t + 1] - pg->rowptr[t]);
                }
              }
  ...
  __device__ void match(Graph* g, PartialGraph* pg, Pattern* pat,
  ...
  __global__ void _parallel_match(Graph* dev_graph, PartialGraph* partial_graph, Pattern* dev_pattern,
    ...
    __shared__ PartialGraph p_graph;
    ...
    while (true) {
      match(&graph, &p_graph, &pat, &stk[local_wid], job_queue, &count[local_wid], &stealing_args);
      __syncwarp();
