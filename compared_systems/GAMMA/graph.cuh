...
/*#ifdef ENABLE_ELABEL
  ...
  void read(std::string file_name, bool read_labels, mem_type mem_tp) {
    std::cout << "Reading graph from file: " << file_name << "\n";
    //need_dag = dag;
    memory_type = mem_tp;
    read_topology_graph(file_name.c_str());
...
/*#ifdef ENABLE_ELABEL
      ...
      std::string vertex_label_file = file_name + std::string(".label.bin");
    ...
    //std::string edge_label_file = file_name + ".elabel";
      ...
      read_subfile(vertex_label_file,labels,nnodes);
      //file.read((char*)labels, sizeof(uint32_t)*nnodes);
      node_data_type *node_data_h = (node_data_type *)malloc(nnodes*sizeof(node_data_type));
      for (size_t i = 0; i < nnodes; i++) {
        node_data_h[i] = static_cast<uint8_t>(labels[i]);
      }
      //memset(node_data_h, 1, nnodes * sizeof(node_data_type));
...
/*#ifdef ENABLE_ELABEL
  ...
  void read_topology_graph(const char file_name[]) { 
    std::string fileneme = file_name;
    std::string meta_file = file_name + std::string(".meta.txt");
    std::string vertex_file = file_name + std::string(".vertex.bin");
    std::string edge_file = file_name + std::string(".edge.bin");
    //std::string weight_file = filename + ".val";

    std::ifstream f_meta(meta_file.c_str());
    assert(f_meta);
    uint32_t n_vertices;
    uint64_t n_edges;
    int vid_size;
